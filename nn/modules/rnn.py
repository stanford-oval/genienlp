...
class RNNBase(Module):
    ...
    def __init__(self, mode, input_size, hidden_size,
        ...
        for layer in range(num_layers):
            for direction in range(num_directions):
                ...
                if mode == 'IndyLSTM':
                    w_hh = Parameter(torch.Tensor(gate_size, 1))
                else:
    ...
    def check_forward_args(self, input, hidden, batch_sizes):
        ...
        if self.mode == 'LSTM' or self.mode == 'IndyLSTM':
...
class IndyLSTM(RNNBase):
    ...
    def __init__(self, *args, **kwargs):
        super(IndyLSTM, self).__init__('IndyLSTM', *args, **kwargs)
...
class IndyLSTMCell(RNNCellBase):
    ...
    def __init__(self, input_size, hidden_size, bias=True):
        super(IndyLSTMCell, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.bias = bias
        self.weight_ih = Parameter(torch.Tensor(4 * hidden_size, input_size))
        self.weight_hh = Parameter(torch.Tensor(4*hidden_size, 1))
        if bias:
            self.bias_ih = Parameter(torch.Tensor(4 * hidden_size))
            self.bias_hh = Parameter(torch.Tensor(4 * hidden_size))
        else:
    ...
    def forward(self, input, hx=None):
        ...
        self.check_forward_hidden(input, hx[0], '[0]')
        self.check_forward_hidden(input, hx[1], '[1]')
        return self._backend.IndyLSTMCell(
            input, hx,
            self.weight_ih, self.weight_hh,
            self.bias_ih, self.bias_hh
        )
