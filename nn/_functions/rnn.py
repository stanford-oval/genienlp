...
def IndyLSTMCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None):

    hx, cx = hidden
    w_hh_diag = torch.cat([torch.diag(chunk.squeeze()) for chunk in w_hh.chunk(4, 0)], dim=0)

    if input.is_cuda:
        print('error: input cannot be on cuda when using IndyLSTM')
        #FIXME: fix this part
        # igates = F.linear(input, w_ih)
        # hgates = F.linear(hidden[0], w_hh_diag)
        # state = fusedBackend.LSTMFused.apply
        # return state(igates, hgates, hidden[1]) if b_ih is None else state(igates, hgates, hidden[1], b_ih, b_hh)

    gates = F.linear(input, w_ih, b_ih) + F.linear(hx, w_hh_diag, b_hh)
