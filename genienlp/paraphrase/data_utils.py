...
def mask_tokens(inputs, labels, tokenizer, mlm_probability, mlm_ignore_index):
    ...
    # print('old inputs = ', [tokenizer.convert_ids_to_tokens(a) for a in inputs.tolist()])
    # print('old labels = ', [tokenizer.convert_ids_to_tokens(a) for a in labels.tolist()])
    labels[~masked_indices] = mlm_ignore_index  # We only compute loss on masked tokens

    # 80% of the time, we replace masked input tokens with tokenizer.mask_token ([MASK])
    output_indices = (torch.cumsum(inputs==sep_token_id, dim=1)-(inputs==sep_token_id).int()).bool()
    # print('output_indices = ', output_indices)
    indices_replaced = torch.bernoulli(torch.full(labels.shape, 1.0)).bool() & masked_indices & output_indices
    # print('indices_replaced = ', indices_replaced)
    inputs[indices_replaced] = tokenizer.pad_token_id

    # 10% of the time, we replace masked input tokens with random word
    # indices_random = torch.bernoulli(torch.full(labels.shape, 0.5)).bool() & masked_indices & ~indices_replaced
    # print('indices_random = ', indices_random)
    # random_words = torch.randint(len(tokenizer), labels.shape, dtype=torch.long)
    # inputs[indices_random] = random_words[indices_random]

    # The rest of the time (10% of the time) we keep the masked input tokens unchanged

    # print('new inputs = ', [tokenizer.convert_ids_to_tokens(a) for a in inputs.tolist()])
    # print('new labels = ', [tokenizer.convert_ids_to_tokens(a) for a in labels.tolist()])
