...
def run_mapping():
    ...
    MAPPING['restaurants'] = {
        'Restaurant': ['@restaurant chain@', 'restaurant', 'food manufacturer', 'business', 'enterprise', 'pizzeria chain'],
        'Person': [
            '@writer@',
            'journalist',
            'author',
        ...
        'postalCode': ['@postal code@', 'administrative territorial entity identifier', 'nominal number', 'unique identifier'],
    }
    ...
    MAPPING['thingpedia'] = {
        ...
        'restaurant_cuisine': ['@cuisine@', '@dish@', '@pasta@', 'food'],
        'restaurant': ['@restaurant chain@', 'restaurant'],
        'Location': ['@city@', 'location', 'lake', 'sovereign state'],
        'device': ['@device@'],
        'iot_name': ['@room@'],
        'dog': ['dog breed'],
    }
    ...
    MAPPING['cross_ner/news'] = {
        ...
        'ORG': ['@organization@', 'business', 'association football club', 'national association football team'],
    ...
    pprint(MAPPING)
    with open(os.path.join(args.bootleg_input_dir, 'es_material/almond_type_mapping.json'), 'w') as fout:
    ...
    with open(os.path.join(args.bootleg_input_dir, 'es_material/almond_type_mapping_matching.json'), 'w') as fout:
        ujson.dump(NEW_MAPPING_match, fout)
    with open(os.path.join(args.bootleg_input_dir, 'es_material/almond_type_mapping_inclusion.json'), 'w') as fout:
...
def run_mapping_2():
    ...
    with open(os.path.join(args.bootleg_input_dir, 'es_material/almond_type2qid.json'), 'w') as fout:
